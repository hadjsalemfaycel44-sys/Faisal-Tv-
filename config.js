...
// ==================== BUILD STREAM URL WITH XTREAM CODE ====================
function buildStreamUrl(channelId) {
    // صيغة URL الإكستريم الصحيحة
    // http://HOST:PORT/live/USERNAME/PASSWORD/CHANNEL_ID/index.m3u8
    return `${HOST}/live/${USER}/${PASS}/${channelId}/index.m3u8`;
}

// ==================== ALTERNATIVE STREAM FORMATS ====================
function getStreamUrlAlternative(channelId) {
    // صيغ بديلة للبث
    const formats = {
        m3u8: `${HOST}/live/${USER}/${PASS}/${channelId}/index.m3u8`,
        ts: `${HOST}/live/${USER}/${PASS}/${channelId}.ts`,
        mp4: `${HOST}/live/${USER}/${PASS}/${channelId}.mp4`
    };
    return formats.m3u8; // نستخدم M3U8 الذي يدعمه المتصفحات
}

// ==================== TEST CONNECTION ====================
async function testStreamConnection() {
    try {
        const testUrl = `${HOST}/live/${USER}/${PASS}/1/index.m3u8`;
        const response = await fetch(testUrl, { method: 'HEAD' });
        
        if (response.ok) {
            console.log('✅ الاتصال بخادم الإكستريم نجح');
            return true;
        } else {
            console.log('❌ فشل الاتصال بخادم الإكستريم');
            return false;
        }
    } catch (error) {
...
function getPlaylistUrl() {
    return `${HOST}/get.php?username=${USER}&password=${PASS}&type=m3u&output=ts`;
}

// ==================== DEBUG INFO ====================
console.log('%c🎬 Xtream Code Configuration', 'color: #FFD700; font-size: 16px; font-weight: bold;');
console.log('🖥️ Host:', HOST);
console.log('👤 User:', USER);
console.log('🔐 Pass: ••••••••');
console.log('📺 Channels:', CHANNELS_DATABASE.channels.length);
