...
function setupEventListeners() {
    ...
    elements.qualityBtn.addEventListener('click', () => {
...
function exitApp() {
    ...
    setTimeout(() => {
        window.close() || alert('تم الخروج من التطبيق');
    }, 1000);
}
...
let controlsTimeout;
elements.playerModal.addEventListener('mousemove', () => {
    elements.playerControls.style.opacity = '1';
    clearTimeout(controlsTimeout);
    
    controlsTimeout = setTimeout(() => {
        if (!elements.videoPlayer.paused) {
            elements.playerControls.style.opacity = '0';
        }
    }, 3000);
});
